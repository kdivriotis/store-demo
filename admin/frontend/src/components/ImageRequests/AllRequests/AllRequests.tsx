...
const AllRequests: FC = () => {
  ...
  const [requests, setRequests] = useState<PartnerImageRequest[]>([]);

  // Get all partners' image change requests from API and set state
  const getImageRequests = useCallback(() => {
    ...
    const url = `${process.env.REACT_APP_API_URL}/admin/partner/image-requests`;
  ...
  if (!isLoading && error && error.trim() !== "") {
    ...
    content = <RequestsGrid requests={requests} onRefresh={getImageRequests} />;
  }

  return (
    <>
      <h1>Αιτήματα Αλλαγής Εικόνας Συνεργατών</h1>
      {content}
    </>
  );
};
