...
import { DeletedProductsGrid } from "../";

import styles from "./DeletedProducts.module.css";

interface ApiResponse {
  products: ProductBrief[];
}

const DeletedProducts: FC = () => {
  const { auth } = useContext(AuthContext) as AuthContextType;
  const { isLoading, error, sendRequest } = useHttp();
  const [products, setProducts] = useState<ProductBrief[]>([]);

  // Get all products from API and set state
  const getProducts = useCallback(() => {
    ...
    const url = `${process.env.REACT_APP_API_URL}/admin/product/deleted`;
  ...
  if (!isLoading && error && error.trim() !== "") {
    ...
    content = (
      <div className="section-padding">
        <p className="text-error">Δεν υπάρχουν προϊόντα.</p>
      </div>
    );
  } else if (!isLoading) {
    content = (
      <DeletedProductsGrid products={products} onRefresh={getProducts} />
    );
  }
