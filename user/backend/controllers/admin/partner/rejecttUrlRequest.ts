 ...
 */
export const rejectUrlRequest = async (
  req: Request,
  res: Response
): Promise<Response> => {
  ...
  try {
    await pool.query(
      `UPDATE partner SET site_url=NULL, site_url_is_verified=0 WHERE BIN_TO_UUID(id)=?`,
      [id]
    );

    return res.status(200).json({
      message: "Ο σύνδεσμος ιστοτόπου του συνεργάτη απορρίφθηκε.",
    });
  } catch (error) {
